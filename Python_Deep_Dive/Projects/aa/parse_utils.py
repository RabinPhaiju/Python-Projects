...
def create_combo_named_tuple_class(fnames,compress_fields):
    ...
    compressed_fields = itertools.compress(field_names,compress_fields)
    return namedtuple('Data',compressed_fields)

def iter_file(fname,class_name,parser):
    # create nemedtuple format
    nt_class = create_named_tuple_class(fname,class_name)
    # get row generator
    reader = csv_parser(fname)
    for row in reader:
        parsed_data = (parser_fn(value) for value,parser_fn in zip(row,parser))
...
def iter_combined_plain_tuple(fnames,class_names,parsers,compress_fields):
    ...
    marged_iter = (itertools.chain.from_iterable(zipped_tuple) for zipped_tuple in zipped_tuples)
    for row in marged_iter:
        compress_row = itertools.compress(row,compress_fields)
        yield tuple(compress_row)
